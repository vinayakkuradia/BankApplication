					...
					<sec:authorize access="hasAnyRole('ADMIN')">
						<c:choose>
							<c:when test="${transaction.transactionStatus=='PENDING'}">
								<td class="form-row"><a class="btn btn-light btn-sm"
									href="approveTransaction?transactionId=${transaction.transactionId}">Approve
								</a> <a class="btn btn-light btn-sm"
									href="rejectTransaction?transactionId=${transaction.transactionId}">Reject</a>
								</td>
							</c:when>
							<c:otherwise>
					        <td>NA</td>
